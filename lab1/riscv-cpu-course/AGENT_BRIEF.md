...
- `src/70-appendix.html` (附錄 A：控制表速查, id `chA`) is GENERATED by `tools/gen-appendix.py` from the real CoreCtrl.v / InstMsg.v; rerun it after adding localparams. It deliberately prints only the 11 casez rows that shipped with the lab. `src/90-widgets.html` clones `#chA` into the floating 「控制表速查」 drawer (button / `?` key).
